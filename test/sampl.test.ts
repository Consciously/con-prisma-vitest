...
test('getPostByID2 should throw an error', async () => {
	prisma.post.findUniqueOrThrow.mockImplementation(() => {
		throw new Error('There was an error.');
	});

	await expect(getPostByID2(1)).rejects.toThrow();
	await expect(getPostByID2(1)).rejects.toThrowError('There was an error');
});
